...
class Galleries extends CI_Controller
{
    ...
    function edit_album()
    {
        ...
        if (isset($user)) {
            ...
            $this->load->model('Galleries_model');
            $edited_id = $this->Galleries_model->editAlbum($_POST, $new_album_dir, $user);

            $photos = $this->Galleries_model->getPhotos($edited_id);
            if (!empty($photos)) {
                foreach ($photos as $photo) {
                    $path = explode('/', $photo->photo);
                    $new_photo_path = $new_album_dir . $path[3];
                    $this->Galleries_model->editPhotoPath($photo->id, $new_photo_path, $user);
                }
            }

            if ($edited_id) {
                $this->session->set_flashdata('updated_id', 1);
            } else {
                $this->session->set_flashdata('error_upload', 'Failed to update Album.');
            }

            redirect('myadminkw/Galleries/edit/' . $edited_id);
        } else {
            redirect('myadminkw');
        }
    }
