<div class="container content">
	...
	if(!empty($members)):
		$tmp_generation = '';
		foreach ($members as $key => $members_item) {
			if(empty($members_item['generation_name'])) continue;

			if($tmp_generation != $members_item['generation_name']){
				$tmp_generation = $members_item['generation_name'];
				echo '<div class="generation-label">'.$members_item['generation_name'].'</div>';
			}
			echo '
